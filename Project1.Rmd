...
Abs.Rel.test <- prop.test(Abs.Rel)
```
Because the p-value is very large (`Abs.Rel.test$p.value`), I do not reject H0. The relative and absolute proportions of *Verrucomicrobia* are not significantly different.
...
Tag.compare.test <- prop.test(Tag.compare)
```
The p-value is very large (`Tag.compare.test$p.value`), so the proportions of *Verrucomicrobia* calculated from CARD-FISH and molecular data are not significantly different.
...
Those error bars are pretty knarly; so I decided to calculate the power for Deep, Inlet, and Outlet proportions. Because the CARD-FISH and molecular data have a different number of observations, I used the smaller observation (CARD-FISH) to calculate power.


**Deep**
```{r, echo=FALSE}
Dep.power <- power.prop.test(n=c(5), p1=Tag.compare[1,1], p2=Tag.compare[2,1], sig.level=0.05, power=NULL, alternative="two.sided")
```


Power = `Dep.power$power`


**Inlet**
```{r, echo=FALSE}
In.power <- power.prop.test(n=c(3), p1=Tag.compare[1,2], p2=Tag.compare[2,2], sig.level=0.05, power=NULL, alternative="two.sided")
```
